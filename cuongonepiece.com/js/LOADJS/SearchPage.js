...
if(typeof GetUrlParameter('textsearch') !== "undefined"){
    textsearch = decodeURIComponent(GetUrlParameter('textsearch'));
}
...
function loadTap(){
    xhttp.open("GET", URLServer+"timtap?textsearch="+textsearch+"&page="+page, false);
    xhttp.send();
}
...
function LoadElement() {
    if(page === 0) page = 1;
    $('.title').empty();
    $('.title').append( "TỪ KHÓA: " + textsearch);
...
function ChosePageSearch(event){
    ...
    if(x.parentNode.className == "number_page"){
        if(x.id != "chose_page"){
            ...
            var state = {}
            var title = '';
            var url = '/search?textsearch='+textsearch+'&page='+page;
