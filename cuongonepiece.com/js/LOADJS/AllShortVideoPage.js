...
function LoadListRelateShortVideoElement() {
    ...
    for(var i = 0; i < list_ralate.length ; i++){
        var elements = '<a  href="/videoedit?index='+list_ralate[i].index+'&backdex='+index+'"><div class="item_shortvideo"><div class="info_item_shortvideo"><div><span>index-'+list_ralate[i].index+'</span><span>'+list_ralate[i].views+' lượt xem</span></div></div><img preload="metadata" src="'+list_ralate[i].url_thumbnail+'"></div></a>';
...
function LoadListNewsShortVideoElement() {
    ...
    for(var i = 0; i < list_temp_news.length ; i++){
        var elements = '<a  href="/videoedit?index='+list_temp_news[i].index+'&backdex='+index+'"><div class="item_shortvideo"><div class="info_item_shortvideo"><div><span>index-'+list_temp_news[i].index+'</span><span>'+list_temp_news[i].views+' lượt xem</span></div></div><img preload="metadata" src="'+list_temp_news[i].url_thumbnail+'"></div></a>';
        $('.contain_shortvideo').append(elements);
    } 
    check_list = 2;
}
function LoadListTopViewShortVideoElement() {
    ...
    for(var i = 0; i < list_temp_views.length ; i++){
        var elements = '<a href="/videoedit?index='+list_temp_views[i].index+'&backdex='+index+'"><div class="item_shortvideo"><div class="info_item_shortvideo"><div><span>index-'+list_temp_views[i].index+'</span><span>'+list_temp_views[i].views+' lượt xem</span></div></div><img preload="metadata" src="'+list_temp_views[i].url_thumbnail+'"></div></a>';
        $('.contain_shortvideo').append(elements);
    } 
    check_list = 3;
}
