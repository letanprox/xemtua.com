module.exports = async (callback, scanner) => {
    ...
    if (index === 'LoadDirect') {
        ...
        let projection = {
            _id: 0, so_tap: 0, so_phim: 0, url_embed: 0, thoi_gian:0
        }
        let select = await scanner["modelxxemtua.com/danhsachlinktap"].dataModel.select(query, projection, {}, 0, 0);
    ...
    if (index === 'LoadEmbed') {
        let query = {so_tap: Number(head_params.get('sotap')) , so_phim:  Number(head_params.get('sophim')) }
        let projection = {
            _id: 0, so_tap: 0, so_phim: 0, url_direct: 0, thoi_gian:0
        }
        let select = await scanner["modelxxemtua.com/danhsachlinktap"].dataModel.select(query, projection, {}, 0, 0);
        select = JSON.parse(JSON.stringify(select[0]));
